...
class PromptManager:
    ...
    def __init__(
        ...
        """
        # Resolve paths relative to python_src directory
        # Find python_src directory by looking for it relative to this file
        current_file = Path(__file__).resolve()
        python_src_dir = current_file.parent.parent  # utils -> python_src
        
        # Try multiple possible locations for prompts directory
        possible_prompts_dirs = [
            python_src_dir / "prompts",  # python_src/prompts
            Path("prompts"),  # Current directory (for backward compatibility)
            Path("python_src/prompts"),  # From root directory
        ]
        
        # Find the first existing prompts directory
        self.prompts_dir = None
        for prompts_path in possible_prompts_dirs:
            if prompts_path.exists():
                self.prompts_dir = prompts_path
                break
        
        # If none found, default to python_src/prompts
        if self.prompts_dir is None:
            self.prompts_dir = python_src_dir / "prompts"
        
        # Resolve file paths relative to python_src directory
        if not Path(prompts_file).is_absolute():
            self.prompts_file = python_src_dir / prompts_file
        else:
            self.prompts_file = Path(prompts_file)
            
        if not Path(best_prompts_file).is_absolute():
            self.best_prompts_file = python_src_dir / best_prompts_file
        else:
            self.best_prompts_file = Path(best_prompts_file)
