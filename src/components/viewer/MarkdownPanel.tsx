      ...
      <Card className="h-full rounded-none border-0 shadow-none">
        ...
        <CardContent className="p-0 h-full">
          <ScrollArea className="h-[calc(100vh-8rem)]">
            <div className={`${isFullWidth ? 'max-w-3xl mx-auto' : ''} p-6 prose prose-slate prose-h1:text-black prose-headings:text-primary prose-p:text-foreground prose-p:font-libertinus prose-strong:text-foreground prose-strong:font-libertinus prose-h1:text-2xl prose-h2:text-xl prose-h3:text-lg prose-h4:text-base prose-h5:text-sm prose-h6:text-xs prose-h2:border-b prose-h2:border-border prose-h2:pb-2 prose-p:leading-relaxed prose-li:font-libertinus prose-table:border-collapse prose-table:border-2 prose-table:border-border prose-table:shadow-soft prose-table:rounded-lg prose-table:overflow-hidden prose-th:border prose-th:border-border prose-th:bg-secondary prose-th:px-6 prose-th:py-4 prose-th:text-left prose-th:font-bold prose-th:text-foreground prose-td:border prose-td:border-border prose-td:px-6 prose-td:py-3 prose-td:text-sm prose-td:align-top prose-td:font-libertinus prose-table:w-full prose-table:my-8 prose-tr:even:bg-muted/30 ${isFullWidth ? '' : 'max-w-none'}`}>
              <ReactMarkdown
